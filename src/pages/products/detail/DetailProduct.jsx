...
export const DetailProduct = () => {
  ...
  return (
    <main className="mx-auto mb-10 flex max-w-7xl flex-col items-center gap-6 px-10 pt-4 md:grid md:grid-flow-dense md:grid-cols-2 md:place-items-start md:gap-10">
      ...
      <h1 className="line-clamp-1 self-start font-primary text-3xl font-bold md:col-span-2">{product?.description}</h1>
      <VariantSwiper
        variants={product?.variants}
        currentVariant={currentVariant}
        setCurrentVariant={setCurrentVariant}
      />
      <div className="hidden w-full rounded-md bg-primary/50 p-4 shadow-md md:col-start-1  md:block">
        <ColorPalette variants={product.variants} currentVariant={currentVariant} />
      </div>
      <section className="min-w-full md:col-start-2 md:row-span-2 md:row-start-3 md:grid md:h-full xl:h-auto xl:gap-20">
        {email && (
      ...
      {coloursArray.map((interno) => {
        ...
        return (
          <div className="relative flex w-full" key={interno.name}>
            ...
            >
              <span className={twMerge("aspect-square w-6 rounded-full")} style={{ background: interno.color }}></span>
              {interno.name}
            </p>
          </div>
        );
